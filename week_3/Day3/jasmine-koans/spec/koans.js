...
describe("the JavaScript language", function(){
  ...
  describe("has different types and operators that", function(){
    ...
    it("considers nulls to be falsy", function(){
        var x = null;
        var result;
        if (x){             //null inside an if evaluates to false. In this case result evaluates to false
           result = true;
        }
        else{
           result = false;
        ...
        expect(result == false).toBeTruthy();// Truthy or Falsy   result evaluates to false
        expect(null === false).toBeFalsy();// Truthy or Falsy     null outside the if isn't equal to false
    ...
    it("knows the type of a function", function(){
        ...
        expect(typeof(xxx)).toBe('undefined');  //with typeof I use 'undefined'. Whit toEqual I use undefined with no quotes.
    });
    ...
    it("has arrays and they can contain anything inside", function(){
        ...
        expect(arr[6]).toEqual(undefined);  //because i didn/t defined de index 6 on the array.Only the index 9
    ...
    it("concatenate arrays - well, kind of", function(){
        ...
        expect(a + b).toEqual('1,2,34,5,6'); //Return a string
    });
...
// Numbers and strings when adding to an array returns a string
   ...
   // each time I compare by reference(== or ===) an array with other one I'm taking on account the position on memory
   //so, a!=b each time
  ...
  describe("considers functions as first class citizens", function(){
    ...
    it("may return arrays that contains functions and so on", function(){
        function example(){
           return [function hola(arg){return [0,10]}];// write the missing code here
        }
    ...
    it("matters, the declaration order when they are anonymous", function(){
        ...
        // The way the functions is declared here requires to declared first exampleB before calling
        //example A
    ...
    it("may return arrays that contains closures and so on", function(){
        function example(){
           return [function hola(arg){return [0,arg+9]}];
        }
    ...
    it("passes arrays by reference", function(){
        ...
        example(x);
      expect(x).toEqual([100,2,3]);//In this case x is pointing to [1,2,3] but on the function was change to 
    });                             //100 on the first index.
    ...
    it("may return a function as the result of invoking a function", function(){
        ...
        expect(example()(1,2)).toEqual(3); //Example holds the add function itself, but without arguments
        var f = example();                 //That's why I used it with arguments. 
  ...
  describe("has multiple ways to define and create objects", function(){
    ...
    it("has a pattern called, the Module Pattern", function(){
        ...
        expect(typeof(obj.points)).toEqual('undefined');   //When to know when is 'undefined' or undefined
    });
    ...
    it("may create objects also with the module pattern", function(){
        function createObject(initialScore, color){
            ...
            return {
                color: color,
                incrementScoreIn: function (plusScore) {
                    score += plusScore;
                },
                points: function () {
                    return score;
  ...
  describe("commons patterns with functions and behaviors", function(){
    ...
    it("hoists variables the way you probably dont expect", function(){
        function generate(){
         ...
         expect(generate()[0]()).toEqual(5);      //Why 5?????????????????????
         expect(generate()[1]()).toEqual(5);      //Why 5 again??????????
    });
